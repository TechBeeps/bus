...
import { CreditCard, PlusCircle, Search, IndianRupee, Phone, User, CheckCircle2, X, Save, ArrowRight, Edit2, Trash2, ShieldCheck } from 'lucide-react';
import { getAdminMonthlyPasses, createAdminMonthlyPass, updateAdminMonthlyPass, deleteAdminMonthlyPass, getCities } from '../services/api';
...
export default function MonthlyPasses() {
  ...
  const openAddModal = () => {
    ...
    const defaultDest = cities.length > 1 ? cities[1].name : (cities[0]?.name || 'Udaipur');
  ...
  const openEditModal = (p) => {
    setEditingPass(p);
    const orig = p.origin_city || p.route?.split('➔')[0]?.trim() || (cities[0]?.name || 'Bari Sadri');
    const dest = p.destination_city || p.route?.split('➔')[1]?.trim() || (cities[1]?.name || 'Udaipur');
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingPass) {
        await updateAdminMonthlyPass(editingPass.pass_id || editingPass.id, payload);
      ...
      const msg = err.response?.data?.detail || 'Failed to save monthly pass';
      showToast?.(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = async (p) => {
    if (!window.confirm(`Are you sure you want to delete pass "${p.pass_id}" for ${p.name}?`)) return;
    try {
      await deleteAdminMonthlyPass(p.pass_id || p.id);
      showToast?.(`Pass ${p.pass_id} deleted successfully`);
      setPasses((prev) => prev.filter((item) => (item.pass_id || item.id) !== (p.pass_id || p.id)));
    } catch (err) {
  ...
  const filteredPasses = passes.filter((p) =>
    ...
    (p.origin_city && p.origin_city.toLowerCase().includes(search.toLowerCase())) ||
    (p.destination_city && p.destination_city.toLowerCase().includes(search.toLowerCase())) ||
    (p.route && p.route.toLowerCase().includes(search.toLowerCase()))
  );

  const totalActivePasses = passes.filter((p) => p.status === 'ACTIVE').length;
  const totalRevenue = passes.reduce((acc, p) => acc + (parseFloat(p.amount) || 0), 0);
  const totalUsedRides = passes.reduce((acc, p) => acc + (parseInt(p.used_rides) || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Issue, monitor, and edit passenger monthly passes with route access, rides tracking, and PIN security.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Pass Revenue</p>
        ...
        <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Active Passes</p>
            <h3 className="text-2xl font-black text-indigo-400 mt-1">{totalActivePasses}</h3>
          </div>
        ...
        {/* 3rd Card: Total Used Rides */}
        <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Rides Used</p>
            <h3 className="text-2xl font-black text-purple-400 mt-1">{totalUsedRides}</h3>
            <p className="text-[10px] text-slate-400 mt-0.5">Passes Redeemed by Passengers</p>
          </div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 p-4 rounded-2xl flex items-center justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            placeholder="Search by passenger name, mobile, route..."
      ...
      <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500">
            ...
            <p className="text-xs mt-1">Click "Add New Monthly Pass" above to issue a pass.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {filteredPasses.map((p) => {
                  ...
                  const remaining = parseInt(p.remaining_rides) || (total - used);
                  const pct = Math.min(100, Math.max(0, Math.round((remaining / total) * 100)));

                  return (
                    <tr key={p.id || p.pass_id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="flex items-center space-x-1.5 font-semibold text-xs text-white">
                          <span>{p.origin_city || p.route?.split('➔')[0] || 'Origin'}</span>
                          <ArrowRight className="w-3 h-3 text-indigo-400" />
                          <span>{p.destination_city || p.route?.split('➔')[1] || 'Destination'}</span>
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="space-y-1.5 w-36">
                          <div className="flex justify-between text-xs font-semibold">
                            ...
                            <span className="text-purple-400">{used} Used</span>
                          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-lg shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <h3 className="text-lg font-bold text-white flex items-center space-x-2">
                ...
                <span>{editingPass ? 'Edit Monthly Pass' : 'Issue New Monthly Pass'}</span>
              </h3>
              <button
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {cities.map((c) => (
                      <option key={c.id || c.name} value={c.name} disabled={c.name === form.origin_city}>
                        {c.name} {c.name === form.origin_city ? '(Same as Origin)' : ''}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, total_rides: parseInt(e.target.value) || 62 })}
                    className="w-full p-3 bg-slate-800/80 border border-slate-700/80 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                  <div>
                    ...
                    <div className="w-full p-3 bg-slate-950/80 border border-slate-800 rounded-xl text-purple-300 text-sm font-mono font-bold flex items-center justify-between">
                      ...
                      <span className="text-[11px] text-emerald-400 font-semibold">{Math.max(0, form.total_rides - form.used_rides)} Remaining</span>
                    </div>
                    <p className="text-[10px] text-slate-500 mt-1">Automatically counted when passenger redeems pass.</p>
                  </div>
              ...
              <div className="flex space-x-3 pt-3">
                ...
                >
                  ...
                  <span>{submitting ? 'Saving...' : editingPass ? 'Update Monthly Pass' : 'Issue Monthly Pass'}</span>
                </button>
              </div>
            </form>
          </div>
